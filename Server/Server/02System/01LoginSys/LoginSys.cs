...
public class LoginSys {
    ...
    public void ReqLogin(MsgPack pack) {
        ...
        if (cacheSvc.IsAcctOnLine(data.acct)) {
            ...
            if (pd == null) {
                ...
                int addPower = (int)(milliseconds / (1000 * PECommon.PowerAddSpace)) * PECommon.PowerAddCount;
                if (addPower > 0)
                {
                ...
                if (power != pd.power)
                {
                    cacheSvc.UpdatePlayerData(pd.id, pd, pack.session);
                }
    ...
    public void ClearOfflineData(ServerSession session)
    {
        ...
        if (pd != null)
        {
            ...
            {
                PECommon.Log("Update offline time error", LogTypeNew.Error);
            }
